<template>
  ...
  >
    <!-- ✅ iPad 3列 / PC 4列 -->
    <div class="grid gap-[1rem] grid-cols-1 sm:grid-cols-2 ipad:grid-cols-3 lg:grid-cols-4">
      <div
        v-for="clip in list"
        :key="clip.id"
        class="group cursor-pointer"
        @click="handleOpen(clip)"
      >
        <!-- thumb -->
        <div
          class="relative w-full aspect-[16/9] rounded-[0.5rem] overflow-hidden
                 border border-[#EAECEF] bg-[#F7F8FA]
        ...
        >
          ...
          <!-- duration badge -->
          <div
            class="absolute top-[0.5rem] left-[0.5rem]
                   bg-[rgba(32,36,47,0.35)] text-white text-[0.75rem] font-['light']
                   w-[3rem] h-[1.75rem] rounded-[0.25rem]
        ...
        <div class="mt-[0.5rem] space-y-[0.25rem]">
          <!-- title -->
          <div
            class="text-[#09090B] text-[1.125rem] font-['medium']
