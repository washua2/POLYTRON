                ...
                <div class="box " :style="{
                    ...
                    <div id="video-hover"
                        ...
                        <div class="absolute top-0 left-0 w-full h-full bg-[rgba(0,0,0,0.7)] flex items-center justify-center gap-[2.875rem] z-[12]"
                            ...
                            <div
                                class="w-[4.5rem] h-[4.5rem] rounded-full bg-[rgba(126,126,126,0.5)] flex items-center justify-center cursor-pointer" @click="handlerPlayBack(index)">
...
const initHoverElement = () => {
    const dropdownVideo = document.getElementById("video-hover");
    document.addEventListener("click", function (event) {
        // 如果点击的不是 dropdown 本身，也不是它的子元素
        if (!dropdownvideo.contains(event.target)) {
            isFocus.value = false
        }
    });
}
